import { BadRequestException, Injectable } from '@nestjs/common';
import { InjectRepository } from '@nestjs/typeorm';
import { InExProYear } from 'src/entity/in_ex_pro_year.entity';
import { Repository } from 'typeorm';
import * as XLSX from 'xlsx';
import { checkNan } from 'src/share/check-nan';

@Injectable()
export class InExProYearService {
  constructor(
    @InjectRepository(InExProYear)
    private readonly iep: Repository<InExProYear>,
  ) {}
  async findAll() {
    try {
      const findAll = await this.iep.find();
      return findAll;
    } catch (err) {
      throw new BadRequestException(err.message);
    }
  }

  async findByDate(date: string): Promise<any> {
    try {
      const findByDate = await this.iep.find({
        where: {
          year: date,
        },
      });
      return findByDate;
    } catch (err) {
      throw new BadRequestException(err.message);
    }
  }

  async findOne(id: number) {
    try {
      const findOne = this.iep.findOne({
        where: {
          id: id,
        },
      });
      return findOne;
    } catch (err) {
      throw new BadRequestException(err.message);
    }
  }

  async importData(file: Express.Multer.File): Promise<any> {
    const workbook = XLSX.read(file.buffer, { type: 'buffer' });
    const sheetName = workbook.SheetNames[0]; // Get the first sheet
    const worksheet = workbook.Sheets[sheetName];
    const jsonData: any[] = XLSX.utils.sheet_to_json(worksheet);

    const formattedData = jsonData.map((row: any) => {
      Object.keys(row).forEach((key) => {
        if (typeof row?.Date === 'number' && row?.Date > 40000) {
          // Likely a date, convert it
          row.Date = XLSX.SSF.format('yyyy', row?.Date);
        }
      });
      return row;
    });

    const mapData = formattedData.map((m) => {
      return {
        branch: m.Branch,
        name: m.nameB,
        year: String(m.Year),
        plan_income: m.Plan_income,
        income: m.Income,
        plan_expenses: checkNan(m.Plan_expense),
        expense: checkNan(m.Expense),
        plan_profit: m.Plan_profit,
        profit: m.Profit,
      };
    });

    //return mapData;

    return await this.iep.save(mapData);
  }
}
